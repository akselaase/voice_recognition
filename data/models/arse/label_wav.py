...
logging.basicConfig(stream=sys.stderr, level=logging.INFO,
                    format='[%(levelname)s]:label_wav.py: %(message)s')
...
def label_wav(labels, graph, input_name, output_name, how_many_labels):
    ...
    logging.info('Loaded tensorflow graph')

    for line in sys.stdin:
        line = line.rstrip()
        if tf.gfile.Exists(line):
            logging.debug('Running tf on {}'.format(line))
            with open(line, 'rb') as wav_file:
                wav_data = wav_file.read()
            run_graph(wav_data, labels_list, input_name,
                      output_name, how_many_labels)
        else:
            logging.warn('File {} didnt exist'.format(line))
            print('File {} didnt exist'.format(line))


def main(_):
    """Entry point for script, converts flags to arguments."""
    label_wav(FLAGS.labels, FLAGS.graph, FLAGS.input_name,
              FLAGS.output_name, FLAGS.num_outputs)
