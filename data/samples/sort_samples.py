...
def find_available_name(dir):
	name = os.path.join(dir, ''.join(random.choices(string.ascii_lowercase, k=6)) + '.wav')
...
def fix_word(word):
	word = word.lower()
	replacements = {'alfa': 'alpha', 'juliet': 'juliett', 'juliettt': 'juliett'}
...
def copy_samples(word):
	...
	target_path = os.path.join('arse', fix_word(word.name))
...
def main():
	for person in os.scandir('arse_raw'):
		if person.is_dir():
