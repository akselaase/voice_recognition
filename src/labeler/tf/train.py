...
def main(_):
    ...
    audio_processor = input_data.AudioProcessor(
        ...
        FLAGS.testing_percentage, FLAGS.max_samples_per_word,
        model_settings, FLAGS.summaries_dir)

    for dataset in ['training', 'validation', 'testing']:
        print('Word counts in', dataset)
        for word in FLAGS.wanted_words.split(','):
            count = sum(
                1 for obj in audio_processor.data_index[dataset] if obj['label'] == word)
            print('   ', word, count)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help='The number of samples from the data set to use for each word.')
    parser.add_argument(
        '--train_dir',
        type=str,
